...
$state->set('selections', [
    ...
    'gender' => '',
    'interests' => [],
    'newsletter' => true,  // 与UI中的checked(true)保持一致
    'notifications' => false
]);
                ...
                Builder::group()
                    ...
                    ->contains(
                        Builder::vbox()->padded(true)->contains([
                            Builder::grid()->form([
                                [
                                    ...
                                    'control' => Builder::combobox()
                                        ...
                                        ->items($countries)
                                        ->selected(0)
                                        ->onChange(function($index, $item, $component, $stateManager) {
                                            $stateManager->set('selections.country', $item);
                                            echo "选择国家: {$item}\n";
                                            
                                            // 更新相关显示
                                            $countryLabel = $stateManager->getComponent('countrySelection');
                                            if ($countryLabel) {
                                ...
                                [
                                    ...
                                    'control' => Builder::combobox()
                                        ...
                                        ->selected(0)
                                        ->onChange(function($index, $item, $component, $stateManager) {
                                            $stateManager->set('selections.language', $item);
                                            echo "选择语言: {$item}\n";
                                            
                                            $languageLabel = $stateManager->getComponent('languageSelection');
                                            if ($languageLabel) {
                            ...
                            Builder::label()->text('中国')->id('countrySelection'),
                            Builder::label()->text('中文')->id('languageSelection'),
                        ])
                    ),
                    ...
                    ->contains(
                        Builder::vbox()->padded(true)->contains([
                            ...
                            Builder::radio()
                                ...
                                ->items(['男', '女', '其他'])
                                ->selected(0)
                                ->onChange(function($index, $item, $component, $stateManager) use ($state) {
                                    $stateManager = $stateManager ?? $state;
                                    $stateManager->set('selections.gender', $item);
                                    echo "选择性别: {$item}\n";
                                    
                                    // 更新显示
                                    $genderLabel = $stateManager->getComponent('genderSelection');
                                    if ($genderLabel) {
                            ...
                            Builder::vbox()->contains(
                                Builder::hbox()->contains([
                                    Builder::checkbox()
                                        ->text('编程')
                                        ->id('interest_0')
                                        ->onChange(function($checked, $component, $stateManager) use ($state, $interestsOptions) {
                                            updateInterests($stateManager ?? $state, $interestsOptions, 0, $checked);
                                        }),
                                    Builder::checkbox()
                                        ->text('设计')
                                        ->id('interest_1')
                                        ->onChange(function($checked, $component, $stateManager) use ($state, $interestsOptions) {
                                            updateInterests($stateManager ?? $state, $interestsOptions, 1, $checked);
                                        }),
                                ]),
                            ),
                    ...
                    ->contains(
                        Builder::vbox()->padded(true)->contains([
                            Builder::checkbox()
                                ...
                                ->text('订阅邮件通知')
                                ->checked(true)
                                ->onChange(function($checked, $component, $stateManager) use ($state) {
                                    $stateManager = $stateManager ?? $state;
                                    $stateManager->set('selections.newsletter', $checked);
                            ...
                            Builder::checkbox()
                                ...
                                ->onChange(function($checked, $component, $stateManager) use ($state) {
                                    $stateManager = $stateManager ?? $state;
                                    $stateManager->set('selections.notifications', $checked);
                        ...
                        Builder::vbox()->padded(true)->contains([
                            ...
                            Builder::button()
                                ...
                                ->onClick(function($button, $stateManager) {
                                    ...
                                    // 更新结果显示
                                    $resultLabel = $state->getComponent('resultLabel');
                                    if ($resultLabel) {
...
function updateInterests($state, $interestsOptions, $index, $checked) {
    ...
    $interestsLabel = $state->getComponent('interestsSelection');
    if ($interestsLabel) {
...
function resetSelections($state) {
    ...
    foreach ($components as $componentId => $defaultValue) {
        $component = $state->getComponent($componentId);
        if ($component) {
    ...
    // 重置复选框
    for ($i = 0; $i < 6; $i++) {
        $checkbox = $state->getComponent("interest_{$i}");
        if ($checkbox) {
    ...
    foreach ($labels as $labelId) {
        $label = $state->getComponent($labelId);
        if ($label) {
            $label->setValue('');
        }
    }
