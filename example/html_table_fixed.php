<?php

require_once __DIR__ . '/../vendor/autoload.php';

use Kingbes\Libui\App;
use Kingbes\Libui\View\Builder;
use Kingbes\Libui\View\HtmlRenderer;
use Kingbes\Libui\View\State\StateManager;

App::init();

// 创建HTML模板文件
$htmlTemplate = <<<'HTML'
<window title="员工管理系统" size="900,600">
    <grid padded="true">
        <label row="0" col="0" colspan="4" align="center" size="large">
            员工信息管理系统
        </label>
        
        <separator row="1" col="0" colspan="4" expand="horizontal"/>
        
        <label row="2" col="0">搜索:</label>
        <input row="2" col="1" bind="searchText" placeholder="输入姓名或职位搜索" expand="horizontal"/>
        <button row="2" col="2" onclick="handleSearch">搜索</button>
        <button row="2" col="3" onclick="handleReset">重置</button>
        
        <separator row="3" col="0" colspan="4" expand="horizontal"/>
        
        <!-- 表格组件 - 使用简单的文本列 -->
        <table row="4" col="0" colspan="4" expand="both" bind="employeeTable">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>姓名</th>
                    <th>职位</th>
                    <th>部门</th>
                    <th>入职时间</th>
                    <th>状态</th>
                </tr>
            </thead>
        </table>
        
        <separator row="5" col="0" colspan="4" expand="horizontal"/>
        
        <hbox row="6" col="0" colspan="4" align="center">
            <button onclick="handleAdd">添加员工</button>
            <button onclick="handleExport">导出数据</button>
            <button onclick="handleRefresh">刷新表格</button>
            <button onclick="handleExit">退出</button>
        </hbox>
        
        <label row="7" col="0" colspan="4" align="center" bind="statusText">
            共5条记录
        </label>
    </grid>
</window>
HTML;

// 创建临时HTML文件
$tempFile = tempnam(sys_get_temp_dir(), 'table_demo_') . '.ui.html';
file_put_contents($tempFile, $htmlTemplate);

// 表格数据
$employeeData = [
    ['001', '张三', '前端工程师', '技术部', '2023-01-15', '在职'],
    ['002', '李四', '后端工程师', '技术部', '2023-03-20', '在职'],
    ['003', '王五', 'UI设计师', '设计部', '2022-11-10', '离职'],
    ['004', '赵六', '产品经理', '产品部', '2023-05-05', '在职'],
    ['005', '钱七', '测试工程师', '测试部', '2023-02-28', '在职']
];

// 事件处理器
$handlers = [
    'handleSearch' => function($button, $state) {
        $searchText = $state->get('searchText');
        if (!empty($searchText)) {
            echo "搜索关键词: {$searchText}\n";
            $state->set('statusText', "正在搜索: {$searchText}");
            
            // 模拟搜索过滤
            $allData = [
                ['001', '张三', '前端工程师', '技术部', '2023-01-15', '在职'],
                ['002', '李四', '后端工程师', '技术部', '2023-03-20', '在职'],
                ['003', '王五', 'UI设计师', '设计部', '2022-11-10', '离职'],
                ['004', '赵六', '产品经理', '产品部', '2023-05-05', '在职'],
                ['005', '钱七', '测试工程师', '测试部', '2023-02-28', '在职']
            ];
            
            $filteredData = array_filter($allData, function($row) use ($searchText) {
                return stripos(implode('', $row), $searchText) !== false;
            });
            
            $state->set('employeeTable', array_values($filteredData));
            $state->set('statusText', '搜索完成，找到 ' . count($filteredData) . ' 条记录');
        } else {
            echo "请输入搜索关键词\n";
        }
    },
    
    'handleReset' => function($button, $state) {
        $state->set('searchText', '');
        $state->set('employeeTable', [
            ['001', '张三', '前端工程师', '技术部', '2023-01-15', '在职'],
            ['002', '李四', '后端工程师', '技术部', '2023-03-20', '在职'],
            ['003', '王五', 'UI设计师', '设计部', '2022-11-10', '离职'],
            ['004', '赵六', '产品经理', '产品部', '2023-05-05', '在职'],
            ['005', '钱七', '测试工程师', '测试部', '2023-02-28', '在职']
        ]);
        $state->set('statusText', '搜索已重置，共5条记录');
        echo "搜索条件已重置\n";
    },
    
    'handleAdd' => function($button, $state) {
        echo "添加新员工\n";
        $state->set('statusText', '正在添加新员工...');
        
        // 模拟添加新员工
        $currentData = $state->get('employeeTable');
        $newId = str_pad(count($currentData) + 1, 3, '0', STR_PAD_LEFT);
        $newEmployee = [$newId, '新员工', '待定', '待分配', date('Y-m-d'), '试用期'];
        $currentData[] = $newEmployee;
        $state->set('employeeTable', $currentData);
        $state->set('statusText', '已添加新员工，共' . count($currentData) . '条记录');
    },
    
    'handleExport' => function($button, $state) {
        echo "导出员工数据\n";
        $state->set('statusText', '正在导出数据...');
        
        $data = $state->get('employeeTable');
        $csv = "ID,姓名,职位,部门,入职时间,状态\n";
        foreach ($data as $row) {
            $csv .= implode(',', $row) . "\n";
        }
        
        // 保存到临时文件
        $tempCsv = tempnam(sys_get_temp_dir(), 'employees_') . '.csv';
        file_put_contents($tempCsv, $csv);
        echo "数据已导出到: {$tempCsv}\n";
        $state->set('statusText', '数据导出完成');
    },
    
    'handleRefresh' => function($button, $state) {
        echo "刷新表格数据\n";
        $state->set('statusText', '表格已刷新');
    },
    
    'handleExit' => function($button, $state) {
        echo "退出应用程序\n";
        App::quit();
    }
];

// 初始化状态
$state = StateManager::instance();
$state->set('searchText', '');
$state->set('employeeTable', $employeeData);
$state->set('statusText', '共5条记录');

// 渲染HTML模板
try {
    $app = HtmlRenderer::render($tempFile, $handlers);
    $app->show();
} finally {
    // 清理临时文件
    if (file_exists($tempFile)) {
        unlink($tempFile);
    }
}


