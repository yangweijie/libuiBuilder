        ...
        if (isset($this->children)) {
            ...
            foreach ($collected as $index => $meta) {
                ...
                if ($colspanExplicit === null) {
                    $colspan = in_array($componentType, ['TableBuilder', 'BoxBuilder', 'MultilineEntryBuilder']) ? $this->getConfig('columns', 12) : 1;
                } else {
                ...
                if (is_string($expandConfig)) {
                    if ($expandConfig === 'both') { $hexpand = 1; $vexpand = 1; }
                    elseif ($expandConfig === 'horizontal') { $hexpand = 1; }
                    elseif ($expandConfig === 'vertical') { $vexpand = 1; }
                } elseif (is_bool($expandConfig)) {
                    if ($expandConfig) { $hexpand = 1; $vexpand = 1; }
                }
                if ($expandConfig === null) {
                    if ($componentType === 'TableBuilder') { $hexpand = 1; $vexpand = 1; }
                    elseif ($componentType === 'ButtonBuilder') { $hexpand = 1; }
                    elseif ($componentType === 'MultilineEntryBuilder') { $hexpand = 1; $vexpand = 1; }
                    elseif ($componentType === 'CheckboxBuilder' || $componentType === 'RadioBuilder') { $hexpand = 1; }
                }

                // alignment defaults
                $alignConfig = $child->getConfig('align', null);
                $halign = Align::Fill; $valign = Align::Fill;
                if ($componentType === 'LabelBuilder') { $halign = Align::Start; $valign = Align::Center; }
                elseif ($componentType === 'EntryBuilder') { $halign = Align::Fill; $valign = Align::Fill; }
                elseif ($componentType === 'ButtonBuilder') { $halign = Align::Fill; $valign = Align::Center; }
                elseif (in_array($componentType, ['TableBuilder','BoxBuilder','MultilineEntryBuilder'])) { $halign = Align::Fill; $valign = Align::Fill; }
                if (is_string($alignConfig)) {
                    $alignMap = ['fill'=>Align::Fill,'start'=>Align::Start,'center'=>Align::Center,'end'=>Align::End,'left'=>Align::Start,'right'=>Align::End];
                    if (str_contains($alignConfig, ',')) {
                        [$hStr,$vStr] = array_map('trim', explode(',', $alignConfig, 2));
                        if ($hStr !== '' && isset($alignMap[$hStr])) $halign = $alignMap[$hStr];
                        if ($vStr !== '' && isset($alignMap[$vStr])) $valign = $alignMap[$vStr];
                    } else { $a = trim($alignConfig); if (isset($alignMap[$a])) { $halign=$alignMap[$a]; $valign=$alignMap[$a]; } }
                }

                $collected[$index] += [
                    'row'=>$row,'col'=>$col,'colspan'=>$colspan,'rowspan'=>$rowspan,
                    'hexpand'=>$hexpand,'vexpand'=>$vexpand,'expandExplicit'=>$expandExplicit,
            ...
            foreach ($collected as $index => $meta) {
                    ...
                    if (!empty($containerChildren)) {
                        ...
                        foreach ($containerChildren as $containerIndex => $containerChild) {
                            ...
                            try {
                                ...
                                $colspan = $containerColspan === null ? $this->getConfig('columns', 12) : $containerColspan;
                                $rowspan = $containerChild->getConfig('rowspan', 1);
                                $hexpand = 0; $vexpand = 0;
                                if ($containerChildType === 'CheckboxBuilder' || $containerChildType === 'RadioBuilder') { $hexpand = 1; }
                                elseif ($containerChildType === 'MultilineEntryBuilder') { $hexpand = 1; $vexpand = 1; }
                                elseif ($containerChildType === 'ButtonBuilder') { $hexpand = 1; }
                                ...
                                Grid::append($this->handle, $containerChildHandle, $col, $row, $colspan, $rowspan, $hexpand, $halign->value, $vexpand, $valign);
                ...
                try {
                    ...
                    $row = $props['row']; $col = $props['col']; $colspan = $props['colspan']; $rowspan = $props['rowspan'];
                    $hexpand = $props['hexpand']; $vexpand = $props['vexpand'];
                    $halign = $props['halign']; $valign = $props['valign'];
                    ...
                    Grid::append($this->handle, $childHandle, $col, $row, $colspan, $rowspan, $hexpand, $halign->value, $vexpand, $valign);
    ...
    public function append(array $components): static
    {
        ...
        foreach ($components as $index => $component) {
            if ($component instanceof ComponentBuilder) {
                // 如果组件已经显式设置了 row 或 col，则尊重用户设置，不要覆盖
